...
const task_collection = document.querySelector('.tasks-elements');
task_collection.style.opacity = 0;
...
if (userTasks.length > 0) {
    ...
    setTimeout(() => {
        ...
        task_collection.style.opacity = 1;
    }, 100);
...
//     if (!tasks_menu.classList.contains("tasks-menu--active")) {
//         document.oncontextmenu = (e) => {
//             e.preventDefault();

//             if (!((event.clientX >= window.innerWidth - 200) || (event.clientY >= window.innerHeight - 115))) {
//                 contextmenu.disabled = false;
...
for (let subtask of subtasks_collection) {
    subtask.onclick = () => {
        subtask.classList.toggle('task-body__list-item--done');
    }
}
...
tasks_submenu.addEventListener("change", () => {
    ...
    else if (many_task_check.checked == true) {
        descr_task_input.value = null;
...
subtask_first_input.addEventListener("input", () => {
    subtask_second_input.placeholder = "Напишите подзадачу";
...
subtask_second_input.addEventListener("input", () => {
    subtask_third_input.placeholder = "Напишите подзадачу";
